...
`keyframe` decodes a video through the system `ffmpeg` binary
...
```rust
let strip = keyframe::extract(Path::new("recording.mp4"), &keyframe::Options::default())?;
for frame in &strip.frames {
    println!("t={}ms held={}ms {} bytes", frame.ts_ms, frame.held_ms, frame.bytes.len());
}
```

Requires `ffmpeg` and `ffprobe` on `PATH`. Without them `keyframe::tooling()`
